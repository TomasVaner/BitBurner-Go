...
std::pair<std::vector<float>, float> NeuralNetwork::predict(const GameState* gameState) {
	...
	const torch::Tensor tProbabilities = results.at(0).cpu();
...
void NeuralNetwork::train(std::vector<Example>& examples, const int batchSize) {
	...
	for (int batchNum = 0; batchNum < batchCount; batchNum++) {
		...
		const torch::Tensor tGames = torch::from_blob(games.data(), {batchSize, GAME_STATE_DATA_SIZE[0], GAME_STATE_DATA_SIZE[1], GAME_STATE_DATA_SIZE[2]}).clone().to(device);
		torch::Tensor tProbabilities = torch::from_blob(probabilities.data(), {batchSize, NUM_MOVES}).clone().to(device);
		torch::Tensor tValues = torch::from_blob(values.data(), {batchSize, 1}).clone().to(device);
		
		std::vector<torch::Tensor> results = net.forward(tGames);
		
		torch::Tensor probabilitiesLoss = -1 * sum(tProbabilities.multiply(results.at(0))) / batchSize;
		torch::Tensor valuesLoss = sum(torch::pow(tValues - results.at(1).view(-1), 2)) / batchSize;
		torch::Tensor totalLoss = probabilitiesLoss + valuesLoss;
