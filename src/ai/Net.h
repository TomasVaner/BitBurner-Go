...
struct NetImpl : public torch::nn::Module {
	torch::nn::Conv3d conv1, conv2, conv3, conv4, convP1, convV1;
	torch::nn::BatchNorm3d bn1, bn2, bn3, bn4, bnP1, bnV1;
	torch::nn::Linear fcP1, fcP2, fcV1, fcV2;
	torch::nn::BatchNorm1d fcBnP1, fcBnV1;

	/**
	 * @brief Constructor which initializes neural net layers
	 */
	NetImpl() :
			conv1(torch::nn::Conv3dOptions(1, 64, 3).stride(1).padding(1)),
			conv2(torch::nn::Conv3dOptions(64, 64, 3).stride(1).padding(1)),
			conv3(torch::nn::Conv3dOptions(64, 64, 3).stride(1).padding(1)),
			conv4(torch::nn::Conv3dOptions(64, 64, 3).stride(1).padding(1)),
			convP1(torch::nn::Conv3dOptions(64, 64, 3).stride(1).padding(1)),
			convV1(torch::nn::Conv3dOptions(64, 64, 3).stride(1).padding(1)),
	 ...
	 */
	std::vector<torch::Tensor> forward(torch::Tensor tensor){
		...
		torch::Tensor probabilities = relu(bnP1(convP1(tensor))); //batchSize by 64 by GAME_STATE_DATA_SIZE[1] by GAME_STATE_DATA_SIZE[2]
		...
		torch::Tensor value = relu(bnV1(convV1(tensor))); //batchSize by 64 by GAME_STATE_DATA_SIZE[1] by GAME_STATE_DATA_SIZE[2]
